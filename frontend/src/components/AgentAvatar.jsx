export function AgentAvatar({ agent, live }) {
  ...
  return (
    <div className="w-40 rounded-xl border border-slate-200 bg-white p-3 shadow-sm focus-within:ring-2 focus-within:ring-blue-500">
      ...
      <span className={`mt-2 inline-block rounded px-2 py-0.5 text-[10px] font-medium ${STATUS_STYLES[status]}`} aria-live="polite">
        {status.toUpperCase()}
      </span>
      <p className="mt-1 truncate text-[11px] text-slate-600" title={detail}>{detail}</p>

      <div className="mt-1 h-1.5 w-full overflow-hidden rounded bg-slate-100" role="progressbar" aria-valuenow={progress} aria-valuemin="0" aria-valuemax="100">
